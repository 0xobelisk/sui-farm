...
module harvest::scarecrow {
    ...
    const TABLE_NAME: vector<u8> = b"scarecrow";
    ...
    public(package) fun delete(user_storage: &mut UserStorage, ctx: &TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        dapp_system::delete_record<DappKey>(dapp_key::new(), user_storage, key_tuple, vector[b"active_until"], ctx);
    }

    public fun get(user_storage: &UserStorage): u64 {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let active_until_raw = dapp_system::get_field<DappKey>(user_storage, key_tuple, b"active_until");
        let mut active_until_bcs = sui::bcs::new(active_until_raw);
        let value = sui::bcs::peel_u64(&mut active_until_bcs);
        value
    }
    ...
    public(package) fun set(user_storage: &mut UserStorage, value: u64, ctx: &mut TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let field_names = vector[b"active_until"];
